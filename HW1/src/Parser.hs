...
happyExpList = Happy_Data_Array.listArray (0,43) ([704,49188,2,0,44,11264,64,16,1024,1024,0,11265,0,44,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseExpr","Expr","Application","Term","VARIABLE","LAMBDA","DOT","LEFTBRACKET","RIGHTBRACKET","%eof"]
        bit_start = st * 12
        bit_end = (st + 1) * 12
...
action_0 (8) = happyShift action_8
...
happyReduction_1 ((HappyAbsSyn4  happy_var_5) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (Variable happy_var_3)) `HappyStk`
	_ `HappyStk`
	(HappyAbsSyn5  happy_var_1) `HappyStk`
	happyRest)
	 = HappyAbsSyn4
		 (Application happy_var_1 (Abstraction happy_var_3 happy_var_5)
	) `HappyStk` happyRest

happyReduce_2 = happyReduce 4 4 happyReduction_2
happyReduction_2 ((HappyAbsSyn4  happy_var_4) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (Variable happy_var_2)) `HappyStk`
	_ `HappyStk`
	happyRest)
	 = HappyAbsSyn4
...
happyReduction_7 (HappyTerminal (Variable happy_var_1))
	 =  HappyAbsSyn6
		 (Var happy_var_1
	)
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	Variable happy_dollar_dollar -> cont 7;
	Lambda -> cont 8;
	Dot -> cont 9;
	LeftBracket -> cont 10;
	RightBracket -> cont 11;
	_ -> happyError' ((tk:tks), [])
	}
